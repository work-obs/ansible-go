...
// SetupModule implements the setup module for gathering system facts
...
func (m *SetupModule) Validate(args map[string]interface{}) error {
	// Setup module has optional filter and gather_subset arguments
	if filterVal, exists := args["filter"]; exists {
		if _, ok := filterVal.(string); !ok {
			return fmt.Errorf("filter argument must be a string")
		}
	}

	if gatherVal, exists := args["gather_subset"]; exists {
		switch gatherVal.(type) {
		case string, []string, []interface{}:
			// Valid types
		default:
			return fmt.Errorf("gather_subset argument must be a string or list")
		}
	}
...
func (m *SetupModule) Execute(ctx context.Context, moduleCtx *plugins.ModuleContext) (map[string]interface{}, error) {
	return RunModule(m, ctx, moduleCtx)
}

// Run executes the setup module
func (m *SetupModule) Run(ctx context.Context, args map[string]interface{}, config *config.Config) (*ModuleResult, error) {
	...
	if m.shouldGather("all", gatherSubset) || m.shouldGather("min", gatherSubset) {
		if err := m.gatherBasicFacts(facts); err != nil {
			return FailResult(fmt.Sprintf("Failed to gather basic facts: %v", err), 1), nil
		}
	}

	// Gather network facts
	if m.shouldGather("all", gatherSubset) || m.shouldGather("network", gatherSubset) {
		if err := m.gatherNetworkFacts(facts); err != nil {
			return FailResult(fmt.Sprintf("Failed to gather network facts: %v", err), 1), nil
		}
	}

	// Gather hardware facts
	if m.shouldGather("all", gatherSubset) || m.shouldGather("hardware", gatherSubset) {
		if err := m.gatherHardwareFacts(facts); err != nil {
			return FailResult(fmt.Sprintf("Failed to gather hardware facts: %v", err), 1), nil
		}
	}
...
func (m *SetupModule) shouldGather(subset string, gatherSubset []string) bool {
...
func (m *SetupModule) matchesFilter(key, filter string) bool {
...
func (m *SetupModule) gatherBasicFacts(facts map[string]interface{}) error {
...
func (m *SetupModule) gatherNetworkFacts(facts map[string]interface{}) error {
...
func (m *SetupModule) gatherHardwareFacts(facts map[string]interface{}) error {
